...
struct _GbSourceVimPrivate
{
  ...
  gulong                   key_press_event_handler_after;
...
gb_source_vim_clear_selection (GbSourceVim *vim)
{
  ...
  gb_source_vim_ensure_scroll_off (vim);
}
...
static void
on_insert_mode_active (GbSourceVim   *vim,
                       GdkEventKey   *key,
...
gb_source_vim_ensure_scroll_off (GbSourceVim *vim)
{
  ...
  scroll = gb_source_vim_adjust_scroll (vim, 0, GB_SOURCE_VIM_ALIGNMENT_NONE);
  buffer = gtk_text_view_get_buffer (GTK_TEXT_VIEW (vim->priv->text_view));
  insert = gtk_text_buffer_get_insert (buffer);
  gtk_text_view_scroll_to_mark (vim->priv->text_view, insert, 0.0,
...
static void
gb_source_vim_key_press_event_after_cb (GtkTextView *text_view,
                                  GdkEventKey *event,
                                  GbSourceVim *vim)
{
      g_print ("after\n");
...
gb_source_vim_connect (GbSourceVim *vim)
{
  ...
  vim->priv->key_press_event_handler_after =
    g_signal_connect_object (vim->priv->text_view,
                             "event-after",
                             G_CALLBACK (gb_source_vim_key_press_event_after_cb),
                             vim,
                             0);
...
gb_source_vim_finalize (GObject *object)
{
  ...
  GbSourceVimPrivate *priv = vim->priv;
  ...
  if (priv->text_view)
    {
      gb_source_vim_disconnect (vim);
      g_signal_handlers_disconnect_by_func(vim->priv->text_view,
                                           G_CALLBACK (on_insert_mode_active),
                                           vim);
      g_object_remove_weak_pointer (G_OBJECT (priv->text_view),
                                    (gpointer *)&priv->text_view);
      priv->text_view = NULL;
